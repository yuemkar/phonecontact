...
if(isset($_POST['grupekle']))
{
  ...
  $grpsql = $db->prepare("select * from grup where grupisim=?");
  $grpsql->execute([$grupisim]); 
  $qrpok = $grpsql->fetch();
  if (isset($qrpok) && is_array($qrpok)){
  	$_SESSION['msg']="Aynı İsimde Grup İsmi Var!";
  }else{
   
  $pre = $db->prepare("insert into grup SET
	grupisim = ?
	");
	$query = $pre->execute(array(
	     $grupisim
	));  
	  header("location:grupekle.php?durum=ok");
  //log tutmal
  $pre2 = $db->prepare("insert into log_admin SET
	...
	$query2= $pre2->execute(array(
	     $_SESSION['id'],
	     'Grup Eklendi id='.$insertid
	)); 	
	
  
  if($query)
  {
  	$_SESSION['msg']="Grup Eklendi";
  }
  }
}
 ...
 <li class="sub-menu">
      ...
      <section id="main-content">
          <section class="wrapper">
            <h3><i class="fa fa-angle-right"></i> Grup Ekleme</h3>,
                  ...
                  <div class="col-md-12">
                      <div class="content-panel">
                          ...
                          <div class="form-group">
                              <label class="col-sm-2 col-sm-2 control-label" style="padding-left:40px;">Grup İsim</label>
                              <div class="col-sm-10">
                                  <input type="text" class="form-control" value=""  name="grupisim" required >
                              </div>
                          </div>
...
<?php //butonlar ?>
                          ...
                          <input type="submit" name="grupekle" value="Onayla" class="btn btn-theme"></div>
<?php //butonlar ?>
                          </form>
                      </div>
                  </div>
     ...
     <section id="main-content">
          <section class="wrapper">
          	<h3><i class="fa fa-angle-right"></i> Grup Listesi</h3>
				<div class="row">
                  ...
                  <div class="col-md-12">
                      <div class="content-panel">
                          <table class="table table-striped table-advance table-hover">
	                  	  	  <h4><i class="fa fa-angle-right"></i> Grup Liste </h4>
	                  	  	  <hr>
                              <thead>
                              <tr>
                                  <th>Say</th>
                              ...
                              <td><?php echo $cnt;?></td>
                                  ...
                                  <td><a href="kullanicilistesi.php?grupid=<?php echo $row['id'];?>"><?php echo $row['grupisim'];?></a></td>
                                   <td>
                                     <a href="grupguncelleme.php?uid=<?php echo $row['id'];?>"> 
                                     <button class="btn btn-primary btn-xs"><i class="fa fa-pencil"></i></button></a>
                                     <a href="grupguncelleme.php?sil=<?php echo $row['id'];?>"> 
